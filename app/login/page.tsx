    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 8, repeat: Infinity }}
          className="absolute -top-40 left-1/3 h-80 w-80 rounded-full bg-orange-500/15 blur-[80px]" />
        <motion.div animate={{ scale: [1, 1.15, 1] }} transition={{ duration: 10, repeat: Infinity, delay: 2 }}
          className="absolute -bottom-40 right-1/3 h-80 w-80 rounded-full bg-blue-500/15 blur-[80px]" />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        <p className="mt-6 text-center text-sm text-zinc-400">
          ...
          <Link href="/signup" className="text-orange-400 transition hover:text-orange-300">Create account</Link>
        </p>
      </motion.div>
    </div>
  )
